...
- **Web Melody Selector Spoiler**: Added a collapsible settings panel in the config portal allowing users to choose their alarm sound. Options include cycling all sounds sequentially (default) or locking to a single melody. Selection triggers a 3-second preview loop on the device buzzer immediately.
- **Dynamic Wi-Fi Settings Layout**: Removed the global Settings spoiler. The Wi-Fi settings form is now conditionally wrapped inside a collapsed spoiler ("Налаштування Wi-Fi") only when connected to the network, and rendered directly as an open form when disconnected.
...
- **Responsive Web Server**: Configured the delay loop to process `webServer.handleClient()` continuously, keeping the web page responsive while sounds are playing.
